...
public class UpdateScheduleServiceBean {
    ...
    public ResponseEntity<ApiResponse> updateSchedule(HttpServletRequest request){
        ...
        ScheduleEntity scheduleFound = scheduleRepository.findByScheduleId(scheduleToSearch);

        if(scheduleFound == null){
            /* if not existing we create it */
            scheduleFound = new ScheduleEntity(userToSearch.getUserId(), dataJson, title, description);
        }
        else{
            /* update schedule */
