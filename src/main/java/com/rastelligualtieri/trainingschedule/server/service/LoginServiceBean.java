...
public class LoginServiceBean {
    ...
    public ResponseEntity<ApiResponse> loginUser(HttpServletRequest request){
        ...
        if(!passwdSentIsCorrect){
            log.warn("[Host: '{}', IP: '{}', Port: '{}', GUID: '{}'] Tried to login but password is not correct.", request.getHeader("Host"),request.getRemoteAddr(),request.getServerPort(), guid);
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
        ...
        return ResponseEntity.ok(ApiResponse.resultOk("/login", "Login sucessfully.",jsonGuid));
    }
}
