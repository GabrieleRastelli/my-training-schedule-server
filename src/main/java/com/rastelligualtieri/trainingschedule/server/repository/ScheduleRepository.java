...
public interface ScheduleRepository extends JpaRepository<ScheduleEntity, String> {
    ...
    @Query("select new com.rastelligualtieri.trainingschedule.server.model.ScheduleGenericInfo(s.scheduleId, s.title,s.description, s.categoria1, s.categoria2, s.equipment, s.creator) from ScheduleEntity s where s.userId = ?1")
    List<ScheduleGenericInfo> findDocumentsForListing(Long userId);

    @Query("select new com.rastelligualtieri.trainingschedule.server.model.ScheduleGenericInfo(s.scheduleId, s.title,s.description, s.categoria1, s.categoria2, s.equipment, s.creator) from ScheduleEntity s join SuggestedScheduleEntity ss on s.scheduleId = ss.scheduleId")
